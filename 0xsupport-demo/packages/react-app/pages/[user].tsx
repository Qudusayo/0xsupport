...
const User: NextPage<{ user: UserProfile }> = ({ user }) => {
  ...
  async function getObfuscatedIdentifier(identifier: string) {
    let obfuscatedIdentifier = (
      await OdisUtils.Identifier.getObfuscatedIdentifier(
        identifier,
        OdisUtils.Identifier.IdentifierPrefix.DISCORD,
        sc!.issuerAddress,
        sc!.authSigner,
        sc!.serviceContext,
        undefined,
  ...
  async function lookupAddress() {
    let obfuscatedIdentifier = getObfuscatedIdentifier(user.id.toLowerCase());
    let attestations =
        ...
        <div
          ...
          style={{
            backgroundImage: user.banner
              ? `url(${user.banner})`
              : "https://miro.medium.com/v2/resize:fit:1358/1*b8qiCBoaRJ0BVCwjJpZzkw.jpeg",
          }}
        >
          <div
...
export const getServerSideProps: GetServerSideProps<{
  ...
  let profilePicture = `https://cdn.discordapp.com/avatars/${user}/${data.avatar}.png?size=1024`;
  let banner = `https://cdn.discordapp.com/banners/${user}/${data.banner}.gif?size=1024`;

  return {
    props: {
      user: {
        fullName: data.global_name,
        profilePicture,
